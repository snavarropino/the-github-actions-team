...
export class HeroService {
  ...
  constructor(private _settingsProvider: AppSettingsProvider,
  ...
  public getHeroes(): Observable<Hero[]> {
    ...
    if (url) {
    return this._httpClient.get<Hero[]>(`${url}/heroes` );
    } else {
  ...
  private getUrl () {
    if ( this._settingsProvider._loaded) {
      return  this._settingsProvider.settings.apiUrl;
    } else {
      return undefined;
    }
  }
  ...
  getHero(id: string): Observable<Hero> {
    const url = this.getUrl();
    if (url) {
    return this._httpClient.get<Hero>(`${url}/heroes/${id}` );
    } else {
      return of(this._heros.find( h => h.id === id ));
    }
  }
