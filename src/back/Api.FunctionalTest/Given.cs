...
namespace Api.FunctionalTest
{
    public class Given
    {
        ...
        public Given InitialHeroes()
        {
            _serverFixture.DbContext.Heroes.AddRange(GetInitialHeros());
            _serverFixture.DbContext.SaveChanges();

            return this;
        }

        private static Hero[] GetInitialHeros()
        {
            return new Hero[]
                {
                    new Hero ()
